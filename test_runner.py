#!/usr/bin/env python3
"""
Integration test runner for ITS Compiler CLI with security test support.
Runs a suite of test templates to validate CLI functionality and security.
"""
...
class TestRunner:
    """Runs integration and security tests for the ITS compiler CLI."""
    ...
    def _get_test_base_url(self) -> str:
        ...
        if not schema_version:
            ...
            print("\n💡 Try:")
            print("  - Update to latest compiler version: pip install --upgrade its-compiler-cli")
    ...
    def get_test_cases(self) -> List[TestCase]:
        ...
        return [
            ...
            # Security Tests with Different CLI Settings
            TestCase(
            ...
            # CLI-specific tests (these were added for CLI functionality)
            TestCase(
                name="CLI: Output to File",
                file_url=f"{base}/templates/01-text-only.json",
                description="Test outputting to file via CLI",
                should_pass=True,
                test_category="cli",
                extra_args=["--output", "/tmp/test_output.txt"],
            ),
            TestCase(
                name="CLI: Verbose Output",
                file_url=f"{base}/templates/02-single-placeholder.json",
                description="Test verbose output flag",
                should_pass=True,
                test_category="cli",
                extra_args=["--verbose"],
            ),
            TestCase(
                name="CLI: No Cache",
                file_url=f"{base}/templates/02-single-placeholder.json",
                description="Test disabling cache via CLI",
                should_pass=True,
                test_category="cli",
                extra_args=["--no-cache"],
            ),
    ...
    def run_test(self, test_case: TestCase) -> TestResult:
        ...
        try:
            ...
            if "cannot find the file specified" in str(e).lower() or "no such file" in str(e).lower():
                ...
                print("  - Install: pip install its-compiler-cli")
                print("  - Use full path: --compiler /path/to/its-compile")
                print("  - Use python module: --compiler 'python -m its_compiler_cli'")
    ...
    def run_all_tests(self, stop_on_failure: bool = False, category_filter: Optional[str] = None) -> bool:
        ...
        if not self.check_compiler_available():
            ...
            print("  1. Install the compiler: pip install its-compiler-cli")
            print("  2. Use full path: --compiler /path/to/its-compile")
            print("  3. Use Python module: --compiler 'python -m its_compiler_cli'")
    ...
    def generate_junit_xml(self, output_file: str) -> None:
        ...
        for category, results in categories.items():
            ...
            testsuite.set("name", f"ITS Compiler CLI {category.title()} Tests")
            ...
            for result in results:
                ...
                testcase.set("classname", f"ITSCompilerCLI{category.title()}Tests")
...
def main() -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Run ITS Compiler CLI integration and security tests")
    parser.add_argument(
        "--compiler",
        default="its-compile",
